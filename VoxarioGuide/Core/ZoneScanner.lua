...
function VG:GetZone(mapID)
    ...
    return self:GetScannedMaps()[mapID] or (VG.Zones and VG.Zones[mapID])
end
...
function VG:ScanZoneRoot(rootID)
    local scanner, maps = self:InitializeZoneScanner(), self:GetScannedMaps(); rootID = tonumber(rootID)
    ...
    self:Debug("ZoneScanner runtime maps: " .. countMaps(maps))
    self:Debug("ZoneScanner saved maps: " .. countMaps(self.db.zoneScanner and self.db.zoneScanner.maps))
...
function VG:ShowZoneStatus()
    local scanner, maps, roots = self:InitializeZoneScanner(), self:GetScannedMaps(), 0; for _ in pairs(scanner.roots) do roots = roots + 1 end
    self:Info(string.format("%s | %s: %d | %s: %d", self:T("ZONE_SCANNER"), self:T("MAPS_DISCOVERED"), countMaps(maps), self:T("ROOTS_SCANNED"), roots))
end
function VG:FindZones(query)
    query = string.lower(query or ""); if query == "" then self:Warn("Usage: /vg zones find <name>"); return end
    local matches, zones = 0, {}; for id, zone in pairs(VG.Zones or {}) do zones[tonumber(id) or id] = zone end; for id, zone in pairs(self:GetScannedMaps()) do zones[tonumber(id) or id] = zone end
...
function VG:ValidateZones()
    local errors = 0; for id, z in pairs(self:GetScannedMaps()) do if tonumber(id) ~= tonumber(z.mapID) or type(z.name) ~= "string" or z.parentMapID == z.mapID then errors = errors + 1 end end; self:Info(string.format("%s: %d errors", self:T("ZONE_DATABASE"), errors))
end
