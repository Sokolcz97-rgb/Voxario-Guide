...
function VG:CreateSettings()
    ...
    frame.lock = CreateFrame("CheckButton", nil, frame, "UICheckButtonTemplate"); frame.lock:SetPoint("TOPLEFT", 16, -46); frame.lock:SetChecked(VG.db.settings.locked)
    frame.lockLabel = frame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall"); frame.lockLabel:SetPoint("LEFT", frame.lock, "RIGHT", 2, 0); frame.lockLabel:SetText(VG:T("LOCK"))
    frame.lock:SetScript("OnClick", function(b) VG.db.settings.locked = b:GetChecked(); frame.lockLabel:SetText(VG:T(b:GetChecked() and "UNLOCK" or "LOCK")) end)
