...
function VG:ApplyGuideMinimizedState()
    ...
    local widgets = { frame.subtitle, frame.counter, frame.stepType, frame.instruction, frame.details, frame.waypoint, frame.previous, frame.skip, frame.next, frame.settings, frame.choose, frame.restart }
...
function VG.UI:RefreshMainFrame()
    ...
    frame.choose:Hide(); frame.restart:Hide(); frame.previous:Show(); frame.skip:Show(); frame.next:Show()
    ...
    if not step then
        ...
        frame.previous:Show(); frame.skip:Hide(); frame.next:Hide(); frame.restart:Show(); frame.choose:Show(); return
    end
